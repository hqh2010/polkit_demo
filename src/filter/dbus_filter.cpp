 ...
 */
bool DbusFilter::isRegularExpression(const QString &expression)
{
 ...
 * @return bool: true: 是 false:否
 */
bool DbusFilter::isMatchFilterList(const QString &data, const QStringList &filterList)
{
    bool isFound = false;
    for (QString item : filterList) {
        if (item == data || (isRegularExpression(item) && isMatchRegExp(data, item))) {
            isFound = true;
            break;
        }
    }
...
bool DbusFilter::isMessageMatch(const QString &name, const QString &path, const QString &interface)
{
    ...
    if (!name.isEmpty() && !isMatchFilterList(name, nameFilter)) {
        return false;
    }
    if (!path.isEmpty() && !isMatchFilterList(path, pathFilter)) {
        return false;
    }
    if (!interface.isEmpty() && !isMatchFilterList(interface, interfaceFilter)) {
        return false;
    }
    return true;
}

bool DbusFilter::loadConfig(const QString &configPath)
{
    return false;
}
