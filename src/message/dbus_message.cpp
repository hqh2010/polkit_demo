...
char *getString(QByteArray buffer, Header *header, quint32 *offset, quint32 endOffset)
{
    ...
    *offset = alignBy4(*offset);
    ...
    // auto val = buffer[*offset];
    // len = read_uint32(header, &val);
...
bool parseHeader(QByteArray buffer, Header *header)
{
    ...
    quint32 endOffset = 0;
    quint8 headerType = 0;
    // quint32 replySerialPos = 0;
    const char *signature;

    // On start, the server generates a random 16 byte nonce and writes it to a file in the user's temporary directory.
    // The nonce file location is published as part of the server's D-Bus address using the "noncefile" key-value pair.
    // After an accept, the server reads 16 bytes from the socket. If the read bytes do not match the nonce stored in
    // the nonce file, the server MUST immediately drop the connection. If the nonce match the received byte sequence,
    // the client is accepted and the transport behaves like an ordinary tcp transport.
    if (buffer.size() < 16) {
        return false;
    }
    ...
    // auto valLength = buffer[4];
    // header->length = read_uint32(header, &valLength);
    ...
    // auto valSerial = buffer[8];
    // header->serial = read_uint32(header, &valSerial);
    ...
    // 9F
    // auto valArrLen = buffer[12];
    // arrayLen = read_uint32(header, &valArrLen);
    ...
    while (offset < endOffset) {
        ...
        case (int)DBusMessageHeaderField::DBUS_MESSAGE_HEADER_FIELD_NUM_UNIX_FDS: {
            ...
            // auto val1 = buffer[offset];
            // header->unixFds = read_uint32(header, (quint8 *)&val1);
    ...
    qInfo() << QString("parseHeader path:%1, interface:%2, member:%3, destination:%4, hasReplySerial:%5, replySerial:%6")
                   .arg(header->path)
